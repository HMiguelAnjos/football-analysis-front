...
function BaseState({ icon, title, description, action, className = '' }: BaseStateProps) {
  return (
    <div className={`surface flex flex-col items-center text-center py-14 px-6 animate-fade-in ${className}`}>
      {icon && (
        <div
          className="mb-4 grid place-items-center w-14 h-14 rounded-2xl
                     bg-white/[0.04] border border-white/[0.06] text-3xl"
          aria-hidden
        >
          {icon}
        </div>
      )}
      <p className="text-slate-100 font-semibold text-[15px] mb-1.5">{title}</p>
      {description && (
        <div className="text-slate-500 text-[13px] leading-relaxed max-w-sm mx-auto">
          {description}
        </div>
      )}
